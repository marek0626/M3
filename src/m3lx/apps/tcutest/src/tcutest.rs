...
use m3::{reply_vmsg, send_recv, wv_assert_eq, wv_assert_ok, wv_require_ok};
...
fn main() -> Result<(), std::io::Error> {
    ...
    let mgate = wv_require_ok!(MemGate::new(0x4000, Perm::RW));
    wv_assert_ok!(t, mgate.write(&buf, 0));
    ...
    if m3::env::args().nth(1).unwrap() == "sender" {
        ...
        let rgate = wv_require_ok!(RecvGate::new_named("chan"));

        for _ in 0..32 {
            let mut msg = wv_require_ok!(recv_msg(&rgate));
            let val: u32 = wv_require_ok!(msg.pop());
            println!("Received {} from {}", val, msg.label());
            wv_assert_ok!(t, reply_vmsg!(msg, Code::Success));
        }
    }
