...
fn test_chan(t: &mut dyn WvTester) {
    let (tx, rx) = wv_require_ok!(msgschan::create());
    let (res_tx, res_rx) = wv_require_ok!(msgschan::create());
    ...
    let act = wv_require_ok!(act.run(|| {
        ...
        let rx0 = wv_require_ok!(msgschan::Receiver::new_bind(source.pop()?));
        let res_tx0 = wv_require_ok!(msgschan::Sender::new_bind(source.pop()?));
...
fn test_chan_macro(t: &mut dyn WvTester) {
    ...
    let act = wv_require_ok!(run_with_channels!(
        act,
