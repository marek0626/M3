...
use m3::{format, wv_assert_eq, wv_assert_err, wv_require_ok, wv_require_some, wv_run_test};
...
fn _hash_empty(
    ...
    wv_require_ok!(hash.finish(&mut buf));
...
fn hash_mapped_mem(t: &mut dyn WvTester) {
    ...
    wv_require_ok!(hash.ep().configure(mcap.sel()));

    // Map memory
    wv_require_ok!(Activity::own()
        .pager()
        .unwrap()
    ...
    wv_require_ok!(tmif::flush_invalidate());
    ...
    wv_require_ok!(Activity::own().pager().unwrap().unmap(ADDR));
}
...
fn seek_then_hash_file(t: &mut dyn WvTester) {
    ...
    wv_require_ok!(file.seek(1 * 1024 * 1024, SeekMode::Cur));
    _hash_file(
        t,
...
fn read0_then_hash_file(t: &mut dyn WvTester) {
    ...
    // Read zero bytes
    let mut buf = [0u8; 0];
    wv_require_ok!(file.read(&mut buf));
...
fn _shake_and_hash(
    ...
    wv_require_ok!(hash.reset(algo));
    wv_require_ok!(mgate.write(SEED.as_bytes(), 0)); // Write seed
    wv_require_ok!(hash.input(0, SEED.len())); // Absorb seed
    wv_require_ok!(hash.output(0, SHAKE_SIZE));
    ...
    wv_require_ok!(hash.reset(&HashAlgorithm::SHA3_256));
    wv_require_ok!(hash.input(0, SHAKE_SIZE));
...
fn shake_and_hash(t: &mut dyn WvTester) {
    ...
    wv_require_ok!(hash.ep().configure(mgate_derived.sel()));
...
fn _shake_and_hash_file(
    ...
    wv_require_ok!(file.hash_output(hash, SHAKE_SIZE));
    wv_require_ok!(file.seek(0, SeekMode::Set));
    ...
    // Write hash to file
    wv_require_ok!(file.seek(0, SeekMode::Set));
    wv_require_ok!(file.hash_output(hash, HashAlgorithm::SHA3_256.output_bytes));

    // Read hash from file
    wv_require_ok!(file.seek(0, SeekMode::Set));
    let mut buf = [0u8; HashAlgorithm::SHA3_256.output_bytes];
    wv_require_ok!(file.read_exact(&mut buf));
    wv_assert_eq!(t, &buf, expected_sha3_256);
}
...
fn shake_and_hash_pipe(t: &mut dyn WvTester) {
    ...
    let closure = wv_require_ok!(act.run(|| {
        ...
        wv_require_ok!(io::stdin().get_mut().hash_input(&hash, usize::MAX));
        wv_require_ok!(io::stdout().get_mut().hash_output(&hash, PIPE_SHAKE_SIZE));
        Ok(())
    }));
    ...
    let hash = wv_require_ok!(HashSession::new("hash1", &HashAlgorithm::SHA3_256));
    {
        ...
        wv_require_ok!(writeln!(ifile, "Pipe!"));
        ipipe.close_writer();
    }
    {
        // hashsum sha3-256
        let mut ofile = wv_require_some!(opipe.reader());
        wv_require_ok!(ofile.hash_input(&hash, usize::MAX));
        opipe.close_reader();
    }
...
fn cshake_nist(t: &mut dyn WvTester) {
    ...
    for test in &CSHAKE_NIST_SAMPLES {
        wv_require_ok!(hash.reset(test.algo));

        // Absorb cSHAKE header
        let size = cshake::prepend_header(&mut buf, test.n, test.s, test.algo.block_bytes);
        wv_require_ok!(mgate.write(&buf[..size], 0));
        wv_require_ok!(hash.input(0, size));

        // Write test data
        wv_require_ok!(mgate.write(test.data, 0));
        wv_require_ok!(hash.input(0, test.data.len()));

        // Check resulting hash
        wv_require_ok!(hash.output(0, test.expected.len()));
        wv_require_ok!(mgate.read(&mut buf[..test.expected.len()], 0));
...
fn kmac_nist(t: &mut dyn WvTester) {
    ...
    for test in &KMAC_NIST_SAMPLES {
        ...
        wv_require_ok!(mgate.write(&buf[..appended_size], test.data.len() as GlobOff));
        wv_require_ok!(hash.input(0, test.data.len() + appended_size));
