...
fn hash_empty(t: &mut dyn WvTester) {
    let mut hash = wv_require_ok!(RoTSession::new("hash", &HashAlgorithm::SHA3_256));
...
fn hash_mapped_mem(t: &mut dyn WvTester) {
    ...
    let hash = wv_require_ok!(RoTSession::new("hash", &HashAlgorithm::SHA3_256));
...
fn _hash_file_start(
    ...
    wv_require_ok!(act.run(|| {
        ...
        let mut hash = wv_require_ok!(RoTSession::new(&format!("hash{}", ty as usize), algo));
        _hash_file(
            &mut t,
            io::stdin().get_mut(),
            &mut hash,
...
fn shake_and_hash(t: &mut dyn WvTester) {
    let mut hash = wv_require_ok!(RoTSession::new("hash", &HashAlgorithm::SHAKE128));
...
fn shake_and_hash_pipe(t: &mut dyn WvTester) {
    ...
    // Create two pipes
    let imgate = wv_require_ok!(MemCap::new(0x1000, Perm::RW));
    let ipipe = wv_require_ok!(IndirectPipe::new(&pipes, imgate));
    let omgate = wv_require_ok!(MemCap::new(0x10000, Perm::RW));
...
fn create_cshake_sess() -> Result<RoTSession, Error> {
    match RoTSession::new("hash", &HashAlgorithm::CSHAKE128) {
