...
fn create(t: &mut dyn WvTester) {
    let rgate = wv_require_ok!(RecvGate::new(math::next_log2(512), math::next_log2(256)));
    wv_assert_err!(
        t,
...
fn send_errors(t: &mut dyn WvTester) {
    let rgate = wv_require_ok!(RecvGate::new(math::next_log2(256), math::next_log2(256)));
...
fn send_recv(t: &mut dyn WvTester) {
    let rgate = wv_require_ok!(RecvGate::new(math::next_log2(512), math::next_log2(256)));
    let sgate = wv_require_ok!(SendGate::new_with(
    ...
    {
        let is = wv_require_ok!(recv_msg(&rgate));
...
fn send_reply(t: &mut dyn WvTester) {
    ...
    // rgate -> reply_gate
    {
        let mut reply = wv_require_ok!(recv_reply(reply_gate, Some(&sgate)));
        let (i1, i2): (i32, i32) = (wv_require_ok!(reply.pop()), wv_require_ok!(reply.pop()));
