...
use m3::{wv_assert_ok, wv_require_ok, wv_run_test};

pub fn run(t: &mut dyn WvTester) {
    wv_run_test!(t, large_pages);
}

fn large_pages(t: &mut dyn WvTester) {
    if let Some(pager) = Activity::own().pager() {
        ...
        wv_assert_ok!(t, pager.map_mem(VIRT, mem.sel(), MEM_SIZE, Perm::RW));
        ...
        wv_assert_ok!(t, pager.unmap(VIRT));
    }
    else {
