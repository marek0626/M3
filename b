...
case "$cmd" in
    ...
    run)
        if [ "$DBG_GEM5" = "1" ]; then
            ./tools/execute/main.py "$crossname" "$script"
        else
            ./tools/execute/main.py "$crossname" "$script" 2>&1 | tee "$M3_OUT/log.txt"
        fi
        ;;

    rungem5)
        M3_RUN_GEM5=1 ./tools/execute/main.py "$crossname" "$script" 2>&1 | tee "$M3_OUT/log.txt"
        ;;
    ...
    dbg=*)
        if [ "$M3_TARGET" = "gem5" ] || [ "$M3_RUN_GEM5" = "1" ]; then
            ...
            ./tools/execute/main.py "$crossname" "$script" "--debug" 1>"$M3_OUT/log.txt" 2>&1 &
            ...
            ./tools/execute/main.py "$crossname" "$script" "--debug" &>/dev/null &
